...
private:
	...
	ObjectState(int time, Event e, int id, const Eigen::Affine3f &pose) :
		timestamp(time), event(e), id(id), pose(pose) {
	}
};
...
public:
	...
	ObjectIdLabeled(const int id, const ObjectLabel &label) :
		id(id), label(label) {
	}
};
...
#include <v4r/changedet/impl/ObjectHistory.hpp>
